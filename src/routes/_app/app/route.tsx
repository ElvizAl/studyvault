...
function RouteComponent() {
  ...
  const handleDeleteNotebook = async (notebookId: string) => {
    try {
      await deleteNotebookFn({ data: { id: notebookId } });
  ...
  const handleRenameNotebook = async (notebookId: string, newName: string) => {
    const name = prompt("Rename notebook:", newName);
    if (!name?.trim()) return;
    try {
      const updated = await updateNotebookFn({
        data: { id: notebookId, name: name.trim() },
      });
      setNotebooks(
        notebooks.map((nb) =>
          nb.id === notebookId ? { ...nb, name: updated.name } : nb,
        ),
      );
    } catch (error) {
